...
unsigned int nTargetSpacing = 50; // 50s
unsigned int nStakeMinAge = 60 * 60 * 5; //5h
...
const string strMessageMagic = "endocoin Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        if (txout.nValue > MAX_MONEY)
            return DoS(100, error("CTransaction::CheckTransaction() : txout.nValue too ENDO"));
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
            ...
            if(nBestHeight == 0)
            {
            nSubsidy = 10000000 * COIN; //premine for pre-sale - bounty - dev
			}
    ...
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
                 ...
                 if(nBestHeight <= 10) 
            {
			     ...
			     else if(nBestHeight <= 40000)
			 {
			     ...
			     else if(nBestHeight <= 43000)
			 {
				 nSubsidy = nSubsidy * 20 ;  //20000% 
			 }
			 
			     else if(nBestHeight <= 80000)
			 {
				 nSubsidy = nSubsidy * 5 ;  //5000% 
			 }
			 
			     else if(nBestHeight <= 85000)
			 {
				 nSubsidy = nSubsidy * 10 ;  //10000% 
			 }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "endocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Edoncoin - Hyperstake for everybody";
        CTransaction txNew;
        txNew.nTime = 1523369615;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523369615;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 321209 : 321209;
        ...
        assert(block.hashMerkleRoot == uint256("0x1f800610568b410ba473fde31ff794eb7bd70482468f459db41ddbbcc583b991"));
